...
file_path = (
    "src/tests/segnet_tuning_30.csv"  # Replace with your actual file path
)
...
study = optuna.create_study(
    direction="minimize"
)  # or "maximize" based on your objective
...
distributions = {
    ...
    "dropout_rate": FloatDistribution(
        0.0, 0.3
    ),  # Example range for dropout rate
    ...
    ),  # Example range for generator training steps
    # Typical kernel sizes
    # Boolean values for batch normalization
...
for _, row in df.iterrows():
    ...
    objective_value = (
        row["Value"] if state == optuna.trial.TrialState.COMPLETE else None
    )
