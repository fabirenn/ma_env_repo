...
def analyze_learning_rate_relationship(data: pd.DataFrame, param_name: str, objective_column: str = 'objective_value'):
    ...
    if len(objective_values) < 2:
        print(f"Not enough data for regression analysis on parameter: {param_name}")
        return {}
    
    # Scatterplot mit Trendlinie
    plt.figure(figsize=(10, 6))
    sns.scatterplot(x=filtered_data['param_value'], y=filtered_data[objective_column], label="Data points")
    ...
    plt.figure(figsize=(10, 6))
    plt.scatter(param_values, objective_values, color='blue', label="Data points")
