...
for param in merged_data['param_name'].unique():
    # Gruppiere die Zielmetrik basierend auf den Werten des aktuellen Hyperparameters
    param_data = merged_data[merged_data['param_name'] == param]
    grouped_data = param_data.groupby('param_value')['objective_value'].apply(list)
    ...
    if len(filtered_groups) > 1:  # Mindestens zwei Gruppen
        ...
        print(f"Hyperparameter '{param}' hat nicht genügend Gruppen für den F-Test.")
...
# F-Values as bars
bars = ax1.barh(f_test_df['Hyperparameter'], f_test_df['F-Value'], alpha=0.6, color='blue', label='F-Value')

# Display F-Values on the bars (with larger font size)
for bar, value in zip(bars, f_test_df['F-Value']):
    ax1.text(value + 0.5, bar.get_y() + bar.get_height() / 2, f'{value:.2f}', va='center', fontsize=14)  # Larger font and more spacing
...
ax1.set_title('U-Net F-Test Results', fontsize=16)  # Larger font for title
ax1.invert_yaxis()  # Largest F-Value at the top
ax1.grid(True, linestyle='--', alpha=0.6)

# P-Values on the second axis
ax2 = ax1.twiny()
points = ax2.plot(f_test_df['P-Value'], f_test_df['Hyperparameter'], 'ro', label='P-Value')

# Display P-Values near the red points (with more spacing and larger font)
for x, y, value in zip(f_test_df['P-Value'], f_test_df['Hyperparameter'], f_test_df['P-Value']):
    ax2.text(x * 1.4, y, f'{value:.1e}', color='red', fontsize=14, va='center', ha='left')  # More spacing with x + 0.5

ax2.axvline(x=0.05, color='green', linestyle='--', label='Significance Threshold (p = 0.05)')  # Significance line
ax2.set_xlabel('P-Value', color='red', fontsize=14)  # Larger font for axis labels
ax2.tick_params(axis='x', labelcolor='red', labelsize=14)  # Larger font for tick labels
ax2.set_xscale('log')  # Log scale for better visibility of small P-Values

# Legend and display
fig.tight_layout()
fig.legend(loc='lower left', fontsize=14)  # Legend in the bottom-right corner with larger font
plt.show()
...
def analyze_learning_rate_relationship(data: pd.DataFrame, param_name: str, objective_column: str = 'objective_value'):
    """
    ...
    Args:
        data (pd.DataFrame): Der Datensatz mit 'param_name', 'param_value' und 'objective_column'.
        param_name (str): Der Name des Hyperparameters (z. B. 'learning_rate').
        objective_column (str): Der Name der Zielmetrik-Spalte.
        
    Returns:
        dict: Ergebnisse der Analyse (Korrelationen, Regressionskoeffizienten, R²-Wert).
    """
    # Filter für die Learning Rate
    ...
    sns.scatterplot(x=filtered_data['param_value'], y=filtered_data[objective_column], label="Datenpunkte")
    sns.regplot(x=filtered_data['param_value'], y=filtered_data[objective_column], scatter=False, label="Trendlinie")
    ...
    spearman_corr, spearman_p = spearmanr(filtered_data['param_value'], filtered_data[objective_column])
    pearson_corr = np.corrcoef(filtered_data['param_value'], filtered_data[objective_column])[0, 1]
    ...
    plt.figure(figsize=(10, 6))
    plt.scatter(param_values, objective_values, color='blue', label="Datenpunkte")
    plt.plot(param_values, predictions, color='red', label=f"Regression (R²={r2:.2f})")
    plt.xlabel(param_name)
    plt.ylabel(objective_column)
...
results = analyze_learning_rate_relationship(merged_data, param_name="learning_rate")
print(results)